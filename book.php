...
<body>
    ...
    <h1>Rudieta Travel Booker</h1>
    <br>
        

    <form action="pay.php" method="get" id="bookingForm">
        ...
        <input type="number" name="flyers" id="flyers" placeholder="Number of travelers" max='10' default='1' required><br>
...
<br><br>
    ...
    if(isset($_GET['from']) && isset($_GET['to'])){
        ...
        if(array_key_exists($from, $prices) && array_key_exists($to, $prices[$from])){
            $priceOptions = $prices[$from][$to];
            $priceLabels = ['7-DAY Trip', '14-DAY Trip', '21-DAY Trip'];

            echo "<h2>Prices from $from to $to:</h2>";
            echo "<ul>";
            for($i = 0; $i < count($priceOptions); $i++){
                $pricePercentage = ($priceModifier / 100);
                $priceModified = $priceOptions[$i] + ($pricePercentage * $priceOptions[$i]);
                echo "<li>{$priceLabels[$i]}: $priceModified €</li>";
            }
            echo "</ul>";
        }
    }
    ...
    if($priceModifier == 0){
        ...
        echo "<h2>Prices right now are lower than usual by $priceModifier %.<br> You should book right now. </h2>";
    }
    else if($priceModifier > 0){
        echo "<h2>Prices right now are higher than usual by $priceModifier %.<br> Maybe try booking later for better pricing.</h2>";
    };
    ?>
